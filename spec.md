...
| 画面 | パス | 内容 |
|------|------|------|
| 入力画面 | `#/input` | カテゴリ選択、テキスト入力、GASへのGET送信 |
...
- 取得時は `action=get` をクエリパラメータに付与する。
...
- GETは `fetch` で通信する。
- CORS制約を避けるため、書き込みもGETで行い、`action=post`、`category`、`content`、`tags` をクエリパラメータで送信する。
- 書き込みレスポンスは受け取らず、送信処理が例外を投げなければ成功として扱う。
...
- `doGet` は `action=post` の場合、受け取ったカテゴリ・本文に `id` と `createdAt` を付与して保存する。
- `doGet` は `action=get` の場合、保存済み記録を新しい順に返す。
...
| 日付 | バージョン | 変更内容 |
|------|-----------|---------|
| 2026-04-18 | 0.1.5 | GAS書き込みをGETのaction=post方式へ変更し、カテゴリ・本文をクエリパラメータで送信する仕様へ更新 |
